...
<section class="content">
    <div class="box">
        ...
        <div class="box-header">
            <h3 class="box-title">Danh sách {{ $name }}</h3>
        </div><!-- /.box-header -->
        <div class="box-body">
            <form class="form-inline filter-post" role="form" method="post">
                ...
                @if(isset($groups))
                    {!! \Form::select('group_id', $groups, '', ['class' => 'form-control', 'placeholder' => '---Tất cả nhóm---', 'id' => 'group-select']) !!}
                @endif
                <button class="btn btn-success" type="submit" id="filter"><i class="fa fa-search fa-fw"></i>Lọc</button>
                <a href="{{ url('admin/category/'.$active.'/create') }}" class="btn btn-info"><i class="fa fa-plus fa-fw"></i>Thêm {{$name}}</a>
            </form>
        </div>
        <div class="box-body">
...
<script type="text/javascript">
    ...
    config.datatable.ajax = {
        url: "{{url('admin/api/category/'.$active)}}",
        type: 'POST',
        data: function(d){
            var filter = {
                name: $('#name-val').val(),
            ...
            @if(isset($groups))
            filter.group_id = $('#group-select').val();
            @endif
    ...
    config.datatable.columns = [
        ...
        },
        {
            ...
            render: function(data, type, row){
                ...
                html += '<a href="{{url("admin/category/$active/update")}}/'+data+'" class="btn btn-warning"><i class="fa fa-edit"></i></button>'
                html += '<a href="{{url("admin/category/$active/delete")}}/'+data+'"  class="btn btn-danger"><i class="fa fa-trash"></i></button>'
                html += '</div>'
                return html;
            }
        }
